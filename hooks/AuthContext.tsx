...
import { APPWRITE_CONFIG, createAppWriteService, MemberInput, MemberRow } from "@/lib/appwrite";
...
type AuthContentType = {
    ...
    register: (email: string, password: string, name: string, phone: string, club: string) => Promise<void>;
    logout: () => Promise<void>;
...
const login = useCallback(
    async (email: string, password: string) => {
      ...
      try {
        ...
        if (!loggedInUser) {
          ...
          const member = await appwriteService.getMemberByUserId(loggedInUser.$id);
  ...
  const register = useCallback(
    async (email: string, password: string, name: string, phone: string, club: string) => {
      setLoading(true);
      setError("");
      try {
        const loggedInUser = await appwriteService.registerWithEmail({email, password, name, phone, club,});
        if (!loggedInUser) {
 ...
 useEffect(() => {
  ...
  const updateMember = useCallback(
    ...
    return (
         <AuthContext.Provider value={{ user, member, loading, error, login, register, logout,refresh, updateMember,}}>
            {children}
